...
function formatDate(dateString: string): string {
    ...
    // Force a specific locale, e.g., 'en-US' for consistency
    const locale = 'en-US';
...
function Table({ datas }: Props) {
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            let response = await fetch('http://localhost:8080/report/delete/' + id, {
                method: "DELETE",
                headers: {
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-blue-600">
            <thead className="bg-gray-50 dark:bg-blue-600">
                <tr key={"01"}>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-white">เบอร์โทร</th>
                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-white">สถานะ</th>
                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-white">วันที่</th>
                    ...
                    datas.map((report, index) => {
                        return (
                            <tr key={report._id + "-" + index}>
                                <td className="px-6 py-4 whitespace-nowrap text-m font-semibold text-gray-800 dark:text-black">{`<${report.item.itemID}> : ${report.item.itemName}`}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-m font-medium text-gray-800 dark:text-black">{report.detail}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-m font-medium text-gray-800 dark:text-black">{report.user.tel}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-m font-medium text-gray-800 dark:text-black text-center">{report.status}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-m font-medium text-gray-800 dark:text-black text-center">{formatDate(report.createdAt)}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <Link className="py-1.5 px-4 inline-flex justify-center items-center gap-2 rounded-md border-2 border-blue-500 font-semibold text-blue-500 hover:text-white hover:bg-blue-500 hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800 mr-3"
                                        href={`./reportPage/${report._id}`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                                    ...
                                    <button className="py-1.5 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-red-500 text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800"
                                        onClick={() => handleDelete(report._id)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
    ...
    return (
        <div className='max-w-6xl min-h-[80vh] m-auto mt-10 '>
            <div className="grid grid-cols-2">
                <div>
                    <h1 className='text-2xl font-bold text-black '>รายการแจ้งซ่อม</h1>
                    <p className='text-gray-400 font-semibold mb-4'>มีรายการแจ้งซ่อมอยู่ <b className="text-gray-500">{item?.length > 0 ? (item.length) : ("No")} </b> รายการ </p>
                </div>
                {/* <div className='flex'>
